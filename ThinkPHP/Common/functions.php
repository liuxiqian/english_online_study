...
/**
 * 下载URL的文件到指定位置
 * @param  stiring  $url     url
 * @param  文件的绝对路径  $file    /Appliction/test.mp3
 * @param  integer $timeout 最大的时间
 * @return true           
 */
function http_copy($url, $file, $timeout=60){
    ...
    if (function_exists('curl_init')){
        ...
        if (curl_error($ch))
        {
            E("curl 出错:" . trigger_error(curl_error($ch)));
        }

        if (!file_put_contents($file, $temp)){
           E("file_put_contents 出错。请检测文件夹" . dirname($file) . "是否可写.");
        }
    } 
    else
    {
