...
// +----------------------------------------------------------------------
// 修正日期：2016.03
// 修正内容：增加附件库支持， 为了增加API支持，将报错提示修改为英文提示
// panjie
namespace Think;
...
class Upload {
    ...
    public function upload($files='') {
        ...
        if(empty($files)){
            $this->error = 'upload file not exist!';
            return false;
        }
        ...
        foreach ($files as $key => $file) {
            ...
            /* 获取文件hash */
            if($this->hash){
                $file['md5']  = $info['md5'] = md5_file($file['tmp_name']);
                $file['sha1'] = $info['sha1'] = sha1_file($file['tmp_name']);
            ...
            /* 对图像文件进行严格检测 */
            $ext = $file['ext'] = strtolower($file['ext']);
            if(in_array($ext, array('gif','jpg','jpeg','bmp','png','swf'))) {
                $imginfo = getimagesize($file['tmp_name']);
                if(empty($imginfo) || ($ext == 'gif' && empty($imginfo['bits']))){
                    $this->error = 'image type forbidden!';
                    continue;
                }
            }
            
            //调用THINK上传驱动前，修改保存文件名，使其具有护展名信息
            $saveNameTemp = $file['savename'];
            $file['savename'] .= '.' . $file['ext'];
            /* 保存文件 如果不存在关联记录，则直接保存文件，否则直接跳过。 */
            if ($this->uploader->save($file,$this->replace)) {
                unset($file['error'], $file['tmp_name']);
                $file['savename'] = $saveNameTemp;
                $info[$key] = $file;
            } else {
                $this->error = $this->uploader->getError();
            }
    ...
    private function setDriver($driver = null, $config = null){
        ...
        if(!$this->uploader){
            E("upload driver not exist: {$name}");
        }
    }
    ...
    private function check($file) {
        ...
        /* 无效上传 */
        if (empty($file['name'])){
            $this->error = 'Can not find file';
            return false;
        }

        /* 检查是否合法上传 */
        if (!is_uploaded_file($file['tmp_name'])) {
            $this->error = 'forbidden file type';
            return false;
        }

        /* 检查文件大小 */
        if (!$this->checkSize($file['size'])) {
            $this->error = 'file size over allowed';
            return false;
        }
        ...
        if (!$this->checkMime($file['type'])) {
            $this->error = 'MIME of file not allowed';
            return false;
        }

        /* 检查文件后缀 */
        if (!$this->checkExt($file['ext'])) {
            $this->error = 'file ext not allowed';
            return false;
        }
    ...
    private function error($errorNo) {
        switch ($errorNo) {
            case 1:
                $this->error = 'The upload file size is over than upload_max_filesize of  php.ini';
                break;
            case 2:
                $this->error = 'The upload file size over than HTML form MAX_FILE_SIZE set value！';
                break;
            case 3:
                $this->error = 'parts of file has uploaded！';
                break;
            case 4:
                $this->error = 'file not be uploaded！';
                break;
            case 6:
                $this->error = 'can not find tmp dir';
                break;
            case 7:
