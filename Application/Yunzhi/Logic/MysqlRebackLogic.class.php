...
class MySQLRebackLogic {

    private $config;                // 配置信息
    private $content;               //备份文本
    private $path = '';          //sql文件存储位置
    private $dbName = array();   //要备份的数据表名
    private $appPath = "";       //application文件夹位置
    ...
    public function __construct($path, $config = array()) {
        ...
        {
            $this->config['DB_HOST'] = C("DB_HOST");
        }        

        if ($this->config['DB_PORT'] === null)
        {
            $this->config['DB_PORT'] = C("DB_PORT");
        }        

        if ($this->config['DB_USER'] === null)
        {
            $this->config['DB_USER'] = C("DB_USER");
        }

        if ($this->config['DB_PWD'] === null)
        {
            $this->config['DB_PWD'] = C("DB_PWD");
        }

        if ($this->config['DB_NAME'] === null)
        {
            $this->config['DB_NAME'] = C("DB_NAME");
        }

        if ($this->config['DB_PREFIX'] === null)
        {
            $this->config['DB_PREFIX'] = C("DB_PREFIX");
        }

        if ($this->config['DB_CHARSET'] === null)
        {
            $this->config['DB_CHARSET'] = C("DB_CHARSET");
        }
    ...
    private function connect() {
        if (mysql_connect($this->config['DB_HOST'] . ':' . $this->config['DB_PORT'], $this->config['DB_USER'], $this->config['DB_PWD'])) {
    ...
    private function setFile() {
        ...
        $fileName = $this->trimPath($this->path . self::DIR_SEP . $recognize . '_' . date('YmdHis') . '_' . mt_rand(100000000, 999999999) . '.sql');
    ...
    public function backup() {
        $this->content = '/* This file is created by www.mengyunzhi.com ' . date('Y-m-d H:i:s') . ' */';
