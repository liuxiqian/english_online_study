...
/**
 * 获取默认值，用于V层的数据为NULL的默认显示
 * @param  混合 $value 
 * @param  string $type  int或string
 * @return 0 或 ""        
 * panjie
 * 2016.04
 */
...
/**
 * 判断value是否存在于二维数组中的某个数组的KEY键中
 * @param   $value  
 * @param  lists $arrays 二维数组
 * @param  键值 $key    
 * @return bool         
 * panjie
 * 2016.04
 ...
 * return true;
 */
function in_array_by_key($value, $arrays, $key)
{
...
/**
 * 获取正确的当前页面 用于只有3页，用户却点击了第5页的情况
 * @param  int $page       用户传入当前页
 * @param  int $pageSize   页面大小
 * @param  int $totalCount 总数据
 * @return int             实际的页码
 */
function getPage($page, $pageSize, $totalCount)
{
    if ((int)$page <= 1)
    {
        return 1;
    ...
    {
        return floor($totalCount / $pageSize) + 1;
    }
}
